document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateHashes = async () => {
        ...
        for (const type of hashTypes) {
            ...
            try {
                if (type === 'MD5') {
                    hashHex = md5(text); // Use our custom MD5 function
                } else {
                    const textAsBuffer = new TextEncoder().encode(text);
                    const hashBuffer = await crypto.subtle.digest(type, textAsBuffer);
                    hashHex = bufferToHex(hashBuffer);
                }
            } catch (error) {
                console.error(`Error generating ${type} hash:`, error);
                hashHex = 'Error generating hash';
            }
            createResultRow(type, hashHex);
        }
    };
    
    // --- Helper function to create a result row in the DOM ---
    const createResultRow = (label, hash) => {
        const row = document.createElement('div');
        row.className = 'hash-result-row';
        
        // This structure matches our new CSS design
        row.innerHTML = `
            <div class="hash-label">${label}</div>
            <div class="hash-content">
                <div class="hash-output">${hash}</div>
                <button class="copy-btn" title="Copy ${label} hash">Copy</button>
            </div>
        `;
        ...
        row.querySelector('.copy-btn').addEventListener('click', () => {
            navigator.clipboard.writeText(hash).then(() => {
                ...
                setTimeout(() => {
                    ...
                    button.style.backgroundColor = ''; // Revert to original theme color
                }, 1500);
            });
        });
    ...
    // --- Event Listener ---
    textArea.addEventListener('input', generateHashes);
    
    // --- Initial Generation for a blank input ---
    generateHashes();
